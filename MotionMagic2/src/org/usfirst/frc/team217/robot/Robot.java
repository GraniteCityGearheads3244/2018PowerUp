...
public class Robot extends IterativeRobot {
	...
	TalonSRX _talon2 = new TalonSRX(6);
	...
	public void robotInit() {
		...
		_talon.config_kF(0, .465, Constants.kTimeoutMs);
		_talon.config_kP(0, 0.4, Constants.kTimeoutMs);
		_talon.config_kI(0, 0, Constants.kTimeoutMs);
		_talon.config_kD(0, 0, Constants.kTimeoutMs);
		/* set acceleration and vcruise velocity - see documentation */
		_talon.configMotionCruiseVelocity(CVel, Constants.kTimeoutMs);
		_talon.configMotionAcceleration(Acc, Constants.kTimeoutMs);
		/* zero the sensor */
		_talon.setSelectedSensorPosition(0, Constants.kPIDLoopIdx, Constants.kTimeoutMs);
		
		
		_talon2.configSelectedFeedbackSensor(FeedbackDevice.CTRE_MagEncoder_Relative, Constants.kPIDLoopIdx, Constants.kTimeoutMs);
		_talon2.setSensorPhase(false);
		...
		_talon2.config_kF(0, .465, Constants.kTimeoutMs);
		_talon2.config_kP(0, 0.4, Constants.kTimeoutMs);
		_talon2.config_kI(0, 0, Constants.kTimeoutMs);
		_talon2.config_kD(0, 0, Constants.kTimeoutMs);
		/* set acceleration and vcruise velocity - see documentation */
		_talon2.configMotionCruiseVelocity(CVel, Constants.kTimeoutMs);
		_talon2.configMotionAcceleration(Acc, Constants.kTimeoutMs);
		/* zero the sensor */
	...
	public void teleopPeriodic() {
		...
		double leftYstick = -1* Math.abs(_joy.getY());
		...
		if (_joy.getRawButton(1)) {
			...
			_talon2.set(ControlMode.MotionMagic, targetPos); 
