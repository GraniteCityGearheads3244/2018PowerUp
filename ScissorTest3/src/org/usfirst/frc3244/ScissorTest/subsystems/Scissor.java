...
public class Scissor extends Subsystem {
 ...
 // member variables to support closed loop mode
    ...
    public Scissor() {
    	...
    	/* Will Do this in the Subsystem Initialization 
		for (talonIndex = 0; talonIndex < kMaxNumberOfMotors; talonIndex++) {
			m_talons[talonIndex].selectProfileSlot(Constants.kSlotIdx, Constants.kPIDLoopIdx);
			m_talons[talonIndex].config_kF(0, 0.2, Constants.kTimeoutMs);
			m_talons[talonIndex].config_kP(0, 0.2, Constants.kTimeoutMs);
			m_talons[talonIndex].config_kI(0, 0, Constants.kTimeoutMs);
			m_talons[talonIndex].config_kD(0, 0, Constants.kTimeoutMs);
		}*/
		
		/* set acceleration and vcruise velocity - see documentation */
		for (talonIndex = 0; talonIndex < kMaxNumberOfMotors; talonIndex++) {
			m_talons[talonIndex].configMotionCruiseVelocity(15000, Constants.kTimeoutMs);
			m_talons[talonIndex].configMotionAcceleration(6000, Constants.kTimeoutMs);
		}
    ...
    public void myMotorsMotionMagic(double targetPos) {
    	motor_Right_Master.set(ControlMode.MotionMagic, targetPos);
    	motor_Left_Slave.set(ControlMode.MotionMagic, targetPos);
    }
    public void myMotorsSTOP() {
    	motor_Right_Master.set(ControlMode.PercentOutput, 0);
    	motor_Left_Slave.set(ControlMode.PercentOutput, 0);
    }
